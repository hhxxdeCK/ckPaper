    ...
    <el-container>
        <el-header>
            <el-breadcrumb style="font-size: 150%" separator-class="el-icon-arrow-right">
                ...
                <el-breadcrumb-item>发布寻物启事</el-breadcrumb-item>
            </el-breadcrumb>
        </el-header>
        <el-main>
            <el-form label-position='left' size="mini" ref="findThingsForm" :rules="rules" :model="findThingsForm" label-width="100px">
    ...
    export default {
        ...
        data() {
            return {
              ...
              lostTime:'',
                ...
                rules:{
                    title:[{ required: true, message: '标题不能为空', trigger: 'blur' }, { max: 10, message: '不能高于十个字'}],
                    category:[{ required: true, message: '失物类别不能为空', trigger: 'blur'}],
                    address:[{ required: true, message: '丢失地点不能为空', trigger: 'blur' }],
                    email:[{ required: true, message: '邮箱地址不能为空', trigger: 'blur' },{type: 'email', message: '请输入正确的邮箱地址', trigger: ['blur', 'change']}],
                    checkNumber:[{ required: true, message: '验证密码不能为空', trigger: 'blur' }],
                    deletePassword:[{ required: true, message: '删除密码不能为空', trigger: 'blur' }],
                    phoneNumber:[{ required: true, message: '手机号不能为空', trigger: 'blur' }, {pattern: /^1[34578]\d{9}$/, message: '目前只支持中国大陆的手机号码',trigger: 'blur'}],
                    amount:[{ required: true, message: '悬赏金额不能为空'}, { type: 'number', message: '悬赏金额必须为数字值'}],
      ...
      methods: {
        ...
        subPicForm() {
          ...
          this.$axios({
            url: "http://localhost:7777/core/pictures",
            method: "post",
            data: this.formData,
          })
            .then(function(res) {
        ...
        onSubmit() {
          ...
          this.$axios({
            method: 'put',
            url: "http://localhost:7777/core/notice",
            data: form,
          }).then( response => {
              this.$refs.upload.submit()
              this.$message({
